...
// ── ELOHIM PATCH (2026-08-09) ────────────────────────────────────────────
// Upstream asserted `None` here (`own_url_for_preflight_unknown_relay_
// returns_none`). That fail-closed behaviour makes a fleet with
// heterogeneous home relays permanently unable to connect across relays.
// The assertion is inverted below; the genuinely-address-less case keeps
// the `None` contract in the test that follows.
#[test]
fn own_url_for_preflight_unknown_relay_falls_back_to_global() {
    ...
    assert_eq!(
        result, global_url,
        "a peer on a relay we do not home to must still be preflighted with \
         our own home-relay URL — it reaches us through OUR relay"
    );
}

#[test]
fn own_url_for_preflight_without_any_own_url_returns_none() {
    let peer_url = Url::from_str(
        "https://unknown-relay.com:443/bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",
    )
    .unwrap();
    ...
    assert_eq!(
        result, None,
        "with no address of our own there is nothing to advertise — this is \
         the only case for which the caller's \"Connection attempted before \
         home relay URL is known\" error is literally true"
    );
}

/// Regression test for the live alpha topology (2026-08-09).
///
/// Wave-2 relay sovereignty homes each conductor to its own doorway's relay:
/// doorway A -> `relay.alpha.elohim.host`, doorway B -> `relay.elohim.host`.
/// A doorway-B conductor (five per-space relays inserted, all on
/// `relay.elohim.host`, plus a global URL on the same relay) must be able to
/// preflight a doorway-A peer. Before the patch this returned `None` and
/// surfaced as "Connection attempted before home relay URL is known" at
/// ~600/min on a conductor whose home relay was in fact confirmed.
#[test]
fn own_url_for_preflight_cross_doorway_relay_split() {
    let eid = test_endpoint_id();
    let our_relay = RelayUrl::from_str("https://relay.elohim.host./").unwrap();
    let our_url =
        Url::from_str(format!("https://relay.elohim.host.:443/{eid}")).unwrap();
    let global_url = Some(our_url.clone());

    // Five DNA spaces, every one homed to doorway B's relay.
    let mut space_relays = HashMap::new();
    for name in [
        b"lamad".as_slice(),
        b"imagodei".as_slice(),
        b"infrastructure".as_slice(),
        b"node-registry".as_slice(),
        b"mishpat".as_slice(),
    ] {
        space_relays
            .insert(space(name), (our_relay.clone(), Some(our_url.clone())));
    }

    // A doorway-A peer, homed to the OTHER relay.
    let peer_url = Url::from_str(
        "https://relay.alpha.elohim.host.:443/bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",
    )
    .unwrap();
    ...
    assert_eq!(
        result, global_url,
        "doorway-B conductor must preflight a doorway-A peer with its own \
         relay.elohim.host URL; failing closed partitions the alpha DHT \
         along the per-doorway relay split"
    );
}
