...
impl IrohTransport {
    ...
    /// URL. If the peer homes to a relay we do not home to, fall back to
    /// our global URL — see the ELOHIM PATCH note in the body.
    ///
    /// Returns `None` only when we genuinely have no address of our own to
    /// advertise, or when the peer URL cannot be parsed at all.
    pub(crate) fn own_url_for_preflight(
        ...
        // Upstream failed closed here ("Peer is on unknown relay, failing
        // preflight"). That makes a fleet with HETEROGENEOUS home relays
        // permanently unable to connect across relays, which contradicts the
        // documented property that a peer URL embeds *that peer's* home relay
        // and the dialing side connects through the other peer's relay.
        //
        // The preflight advertises OUR OWN address so the remote can address
        // us back; the remote reaches us through OUR home relay, which it
        // learns from exactly this URL. Whether the remote's own home relay
        // happens to be one of ours is irrelevant to our addressability.
        //
        // Live evidence (alpha, 2026-08-09): conductors homed to
        // relay.elohim.host (doorway B) failed 100% of initiations to peers
        // homed to relay.alpha.elohim.host (doorway A) — reported through the
        // caller's misleading "Connection attempted before home relay URL is
        // known", even though the home relay was confirmed and every
        // per-space relay had been inserted.
        //
        // Refusing is still correct when we have NO address of our own; that
        // is the remaining `None` below, and it is the only case for which
        // the caller's error message is literally true.
        if let Some(global) = global_url {
            info!(
                %peer_url,
                %peer_relay,
        ...
        warn!(
            %peer_url,
            %peer_relay,
            "No home-relay URL of our own is known yet, failing preflight"
        );
        None
    }
